...
TEST(MathArithmetics, throw_when_expression_is_operator) {
...
TEST(MathArithmetics, can_get_integer_const) {
	MathArithmetics expression("1+2");
	expression.ToPostfix();
	EXPECT_EQ(3, expression.Calculate());
}

TEST(MathArithmetics, can_get_double_const) {
	MathArithmetics expression("2.6-1.5");
	expression.ToPostfix();
	EXPECT_EQ(1.1, expression.Calculate());
}

TEST(MathArithmetics, expression_with_different_order_are_not_equal_in_div) {
	vector<double> example = { 1,2 };
